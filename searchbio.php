...
$term = $_POST["otherCancer"];
$disease = str_replace("+", " ", $term);

$file_name = dirname($_SERVER['SCRIPT_FILENAME'])."/bio_result/bio_cancer.txt";
...
if ($subty !='') {
	echo "<h2>Biomarkers for ".$disease.": $subty</h2>";

	if( $intervention !='') {
		
		echo "<b>Intervention:  $intervention</b> <p>";
	}
	$disease = $subty;
}
else {
	echo "<p><b>Biomarkers: ".$disease."</b>";
	if (trim($_POST["otherBio"]) == '') {
		echo "<br>(only biomarkers with citations are shown)";
	}
	echo "<p>";
	if( $intervention !='') {		
		echo "<b>Intervention:  $intervention</b> <p>";
...
$inter_arr = array();
if ($intervention  != '') {
	if(strpos($intervention, ",") ) {
		$inter_arr = explode(",", $intervention);
		foreach ($inter_arr as $invla) {
			if( trim($invla) != '') {
				$invla = $invla."[title/abstract]";
			}			
		}
		$temp = implode(" OR ", $inter_arr);
		$string .= " AND (".$temp.") "; 
	}
	else {
		$string .= " AND ".$intervention."[title/abstract]"; 
	}
}
	$bio1 = '';
...
$bio_array = array();
	...
	if (trim($_POST["otherBio"]) != '') {
	$bio1 = trim($_POST["otherBio"]);
	$bio1_flag = 0;
	$bio_array = explode(",", $bio1);
	foreach ($bio_array as $bioa_v) {
		if (trim($bioa_v) != '') {
		$bioa_1 = trim($bioa_v);
		foreach ($word_array as $value) {		
			for($j=0; $j<sizeof($value); $j++){
				if (strtolower($value[$j]) == strtolower($bioa_1)) {
					$bio1_flag = 1;
					array_push($word_array1, array($value));
				}
			}
		}
		}
	...
	if ($part_string != '') {
		...
		$part_string0 = urlencode("$disease, $intervention AND ( $buffer, $part_string0)");
	}
	else {
			$string2 = urlencode($string." AND \"".$buffer."\"[Title/Abstract]");
			$part_string0 = urlencode("$disease, $intervention AND $buffer");
	}
	...
	fclose($handle1);
		...
		die();
	} // else no POST[otherBio]
		
	 $totleArr = array();
	 $totleArr0 = array();
	 
	foreach ($word_array1 as $value_a) {
		...
		foreach ($value_a as $value)	{
			...
			array_push($totleArr, "{{".$part_string."}}");
			array_push($totleArr0, "(".$part_string0.")");
		}
		$part_string = implode(" AND ", $totleArr);
		$part_string0 = implode(" AND ", $totleArr0);
		$alias = $part_string0;
		
	}
	
	if ($part_string != '') {
		$string2 = urlencode($string." AND {{\"".$bio1."\"[Title/Abstract] OR $part_string }}");
		$part_string0 = urlencode("$disease $intervention AND ( $bio1, $part_string0)");
	}
	else {
		$string2 = urlencode($string." AND \"".$bio1."\"[Title/Abstract]");
		$part_string0 = urlencode("$disease $intervention AND $bio1");
	}
	...
	echo "<b> <a href=search.php?ooterm=$part_string0&term=$string2 target=new>$bio1</a> </b> <br>";
...
function searchQue($query) {
  ...
  $esearch = "$utils/esearch.fcgi?" .
                "db=pubmed&retmax=1&usehistory=y&db=pubmed&tool=pmhh&email=pubmedhh@nlm.nih.gov&api_key=$ncbi_key".$query;
