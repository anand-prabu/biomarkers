	...
	switch ($tagName) {
	  case "PUBDATE":
		...
		case "PUBLICATIONTYPE":
		  $this->final[$i]["plist"][] = trim($this->final[$i]["ptype"]);
		  $this->final[$i]["ptype"] = "";
			break;
		case "VERNACULARTITLE":
...
function everything ($url) {
  ...
  while ($data = fread($fp, 8192)) {
    // Parse each chunk with the XML parser created above
...
for ($j=0; $j<$numarti; $j++) {
		...
		if (is_array($this->final[$j]["plist"]) == true) {
		...
		if (is_array($this->final[$j]["mesh"]) == true) {
		...
		$this->final[$j]["linkout"] = trim($this->final[$j]["linkout"]);
		$this->final[$j]["pmid"] = trim($this->final[$j]["pmid"]);
		$this->final[$j]["pdate"] = trim($this->final[$j]["pdate"]);
		$this->final[$j]["pinfo"] = trim($this->final[$j]["pinfo"]);
		$this->final[$j]["atitle"] = trim($this->final[$j]["atitle"]);
		$this->final[$j]["abtext"] = str_replace(array("\r","\n","\t"), "", $this->final[$j]["abtext"]);
		$this->final[$j]["abtext"] = trim(htmlentities($this->final[$j]["abtext"]));
		$this->final[$j]["author"] = trim($this->final[$j]["author"]);
		$this->final[$j]["cname"] = trim($this->final[$j]["cname"]);
		$this->final[$j]["pt"] = trim($this->final[$j]["pt"]);
		$this->final[$j]["jtitle"] = trim($this->final[$j]["jtitle"]);
		$this->final[$j]["allmesh"] = trim($this->final[$j]["allmesh"]);
		$this->final[$j]["issn"] = trim($this->final[$j]["issn"]);
		$this->final[$j]["vertitle"] = trim($this->final[$j]["vertitle"]);
}			
